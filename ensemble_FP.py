...
def ensemble_FP(Ens_ten, observation_func, y_true_mean, y_true_std, alpha, beta, gamma=1):
    ...
    Ens_ten_out = Ens_ten.squeeze(2) + gamma*torch.transpose(torch.mm(K, (torch.transpose(y_true_mean,0,1) - Y + alpha*R_2.transpose(0,1))), 0, 1) # ens by x + (x by y)*(1 by y - y by ens + y by ens)
...
def ensemble_FP_cov(Ens_ten, observation_func, y_true_mean, y_true_cov, alpha, beta):
    ...
    K = torch.linalg.solve(y_true_cov + C_yy, C_xy, left=False)
    ...
    Ens_ten_out = Ens_ten.squeeze(2) + torch.transpose(torch.mm(K, (torch.transpose(y_true_mean,0,1) - Y.mean(1).unsqueeze(1) + alpha*R_2)), 0, 1) # ens by x + (x by y)*(1 by y - y by ens + y by ens)
