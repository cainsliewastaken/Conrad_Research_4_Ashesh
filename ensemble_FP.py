...
def ensemble_FP(Ens_ten, observation_func, y_true_mean, y_true_std):
    ...
    C_xy = (torch.mm(X, torch.transpose(Y, 0, 1)))/(Ens_ten.shape[0]-1) # shape x by y
    C_yy = (Y @ Y.transpose(0,1))/(Ens_ten.shape[0]-1) # shape y by y
    ...
    K = torch.mm(C_xy, torch.inverse(R + C_yy))
        # shape (x by y) * [(y by y) + (y by y)]

    # print('Input norm max and min ', torch.linalg.vector_norm(Ens_ten, dim=0).max().cpu().numpy(), torch.linalg.vector_norm(Ens_ten, dim=0).min().cpu().numpy() ,'X norm max and min ', torch.linalg.vector_norm(X, dim=0).max().cpu().numpy(), torch.linalg.vector_norm(X, dim=0).min().cpu().numpy())
    # print('Y norm max and min ', torch.linalg.vector_norm(Y, dim=0).max().cpu().numpy(), torch.linalg.vector_norm(Y, dim=0).min().cpu().numpy(),'Y cov max and min ', C_yy.max().cpu().numpy(), C_yy.min().cpu().numpy())
    # # print('Cond number of R, C_yy, (R + C_yy) and C_xy', np.linalg.cond((R).cpu().numpy()), np.linalg.cond(C_yy.cpu().numpy()), np.linalg.cond((R+C_yy).cpu().numpy()), np.linalg.cond(C_xy.cpu().numpy()))
    # print('Mean ens max and min', Ens_ten.mean(0).max().cpu().numpy(),  Ens_ten.mean(0).min().cpu().numpy(), 'X mean', x_m.mean().cpu().numpy())
    # print('')
    ...
    # Ens_ten_out = Ens_ten.squeeze(2) + torch.transpose(torch.mm(K, (torch.transpose(y_true_mean,0,1) - Y + R_2.transpose(0,1))), 0, 1) # ens by x + (x by y)*(1 by y - y by ens + y by ens)

    return Ens_ten_out.unsqueeze(2)


def ensemble_FP_cov(Ens_ten, observation_func, y_true_mean, y_true_cov):
    # Ens_mat is the tensor of ensembles, with ens x (spacial)
    Y = observation_func(Ens_ten) # shape ens by y
    ...
    # Ens_ten_out = Ens_ten.squeeze(2) + torch.transpose(torch.mm(K, (torch.transpose(y_true_mean,0,1) - Y + R_2)), 0, 1) # ens by x + (x by y)*(1 by y - y by ens + y by ens)
