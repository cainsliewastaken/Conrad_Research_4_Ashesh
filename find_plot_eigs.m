...
direct_step_jacs_FNO_untrained =load('Model_output\FNO_KS_Directstep_tendency_lead1_UNTRAINED_jacs.mat');
PEC_step_jacs_FNO_untrained =load('Model_output\FNO_KS_PECstep_tendency_lead1_UNTRAINED_jacs.mat');


[v_direct_un, e_direct_un] = eig(squeeze(direct_step_jacs_untrained.Jacobian_mats(2,:,:)));
...
[v_direct_FNO_un, e_direct_FNO_un] = eig(squeeze(direct_step_jacs_FNO_untrained.Jacobian_mats(2,:,:)));
...
hold on
histogram(abs(e_direct_un))
legend('Direct Eigvals','Direct untrained',fontsize=10)
...
% binCenters = h.BinEdges + (h.BinWidth/2);
%plot(binCenters(1:end-1), p)
histogram(abs(e_PEC-1)/1e-3)
hold on
histogram(abs(e_PEC_un-1)/1e-3)
legend('PEC Eigvals','PEC untrained',fontsize=10)
...
% binCenters = h.BinEdges + (h.BinWidth/2);
%plot(binCenters(1:end-1), p)
histogram(abs(e_PEC_FNO-1)/1e-3)
hold on
histogram(abs(e_PEC_FNO_un-1)/1e-3)
legend('PEC FNO Eigvals','PEC FNO untrained',fontsize=10)
