...
rand_indices = torch.cat([torch.tensor([0]),torch.randint(1, 10000, [num_ensembles-1])])
# print(rand_indices)
noised_input = label_test_torch[rand_indices,:].cuda()
    ...
    #  print('time step',k)
        if (k==0):
            ...
            if (k%1000==0):
                ...
                print(np.sqrt(np.mean((net_pred[k-1,0] -  label_test[k-1])**2)), float(output.std(0).mean()))
